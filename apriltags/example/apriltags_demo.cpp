...
public:
  ...
  void setup() {
	...
	//add by fan6 for socket connection.//
...
//	  char *servInetAddr="127.0.0.1";
	  sockaddr.sin_addr.s_addr=inet_addr("127.0.0.1");//服务器IP地址  
	sockaddr.sin_family = AF_INET;
	sockaddr.sin_port = htons(7000);
	//printf("%d\n",INADDR_ANY);
//	sockaddr.sin_addr.s_addr = htonl(INADDR_ANY);
//	inet_pton(AF_INET,servInetAddr,&server_sockaddr.sin_addr)
		if((connect(ss,(struct sockaddr*)&sockaddr,sizeof(sockaddr))) < 0 )
		{
  ...
  void print_detection(AprilTags::TagDetection& detection) const {
    ...
    cout << "  distance=" << translation.norm()
         ...
         << ", roll=" << roll
         << endl;
//add by fan6 to send socket.//
int locataon[2];
double x=-translation(1);
double y=translation(2);
double X=-x*cos(yaw)-y*sin(yaw);
double Y=-y*cos(yaw)+x*sin(yaw);
locataon[0]=(int)(X*100);
locataon[1]=-(int)(Y*100);

char transend='\n';
		 cout << "	loca_X= " << locataon[0] <<" loca_Y= "<< locataon[1]<<endl;
		 send(ss, locataon, sizeof(locataon) , 0);
		 send(ss, &transend, sizeof(transend) , 0);
//	send(ss, &translation(1), sizeof(translation(1)) , 0);
//	send(ss, &translation(2), sizeof(translation(2)) , 0);
//    char  sendline[11]={'a','p','r','i','l','f','i','n','d','\r',\n'};
//	send(ss, sendline, 11 , 0);
  ...
  void processImage(cv::Mat& image, cv::Mat& image_gray) {
    ...
    if (m_arduino) {
      ...
      0,  0,  1;
    	...
    	double yaw, pitch, roll;
    	wRo_to_euler(fixed_rot, yaw, pitch, roll);
		double X_room,Y_room;
		X_room=translation(1)*cos(yaw)-translation(2)*sin(yaw);
		Y_room=translation(2)*cos(yaw)+translation(1)*sin(yaw);
		//end 
		...
		m_serial.print(",");
        m_serial.print(X_room);
		m_serial.print(",");
        m_serial.print(Y_room);
		m_serial.print(",");
        m_serial.print(yaw);
		m_serial.print("\n");
